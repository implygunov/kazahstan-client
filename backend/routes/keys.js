...
router.post('/admin/keys/generate', authMiddleware, adminOnly, (req, res) => {
...
router.post('/admin/keys/delete', authMiddleware, adminOnly, (req, res) => {
  const { keyId } = req.body;
  const idx = data.keys.findIndex(k => k.id === parseInt(keyId));
  if (idx === -1) return res.status(404).json({ error: 'not_found' });
  data.keys.splice(idx, 1);
  save();
  logAction('key_delete', `keyId:${keyId}`, '', req.user);
  res.json({ ok: true });
});
